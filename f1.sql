...
CREATE TABLE IF NOT EXISTS PIT_STOPS (raceId INTREGER REFERENCES RACES (raceId) ON DELETE CASCADE ON UPDATE CASCADE, driverId INTEGER REFERENCES DRIVERS (driverId) ON DELETE CASCADE ON UPDATE CASCADE, stop INTEGER, lap INTEGER, time TIME, duration FLOAT, milliseconds DOUBLE, PRIMARY KEY (raceId, driverId, time));